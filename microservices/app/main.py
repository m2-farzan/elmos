...
def is_affected_by_jafarfard_bug():
  if int(session['user_dep_id']) == 16:
    return False
  cur = mysql.connection.cursor()
  bug = cur.execute("select * from picks_2 where user_id=%s and unit_id=161115001", [session['user_id']])
  return bug != 0
...
"""))
  if is_affected_by_jafarfard_bug():
    flash(('red', """
در پی گزارش یکی از کاربران، معلوم شد که درس «فیزیک ۲» استاد «جعفرفرد» با کد «161115001» فقط برای رشته فیزیک ارائه شده است. متاسفانه الگوریتم سایت این مورد را تشخیص نداده بود و یادداشت «مخصوص فیزیک» را در کنار اسم درس قرار نداده بود.

متاسفانه شما یکی از ۱۲ نفری هستید که این باگ روی برنامه‌تان اثر گذاشته است. لطفا برنامه خود را اصلاح کنید.
